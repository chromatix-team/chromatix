...
def thick_polarised_sample(field: cf.VectorField, potential: ArrayLike, n_background: ArrayLike, dz: ArrayLike, NA: float = 1.0) -> cf.VectorField:
    ...
    # Getting k_grid 
    k_grid = 2 * jnp.pi * jnp.stack(jnp.meshgrid(jnp.fft.fftfreq(n=padded_shape[0], d=field.dx.squeeze()), jnp.fft.fftfreq(n=padded_shape[1], d=field.dx.squeeze()), indexing='ij'))[:, None, ..., None, None]
    ...
    Q = jnp.where(jnp.sum(k_grid[1:, ..., None]**2, axis=0) <= NA**2 * km**2, Q, 0) # Limit NA 
