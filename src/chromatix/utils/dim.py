"""
Code ported from [this file in MacroMax](https://github.com/tttom/MacroMax/blob/master/python/macromax/utils/dim.py) to
work with JAX.
"""
import jax
...
array_like = Union[Complex, Sequence, jax.Array]
...
def add(arr: array_like, left: Optional[int] = None, right: Optional[int] = None, ndim: Optional[int] = None
    ...
    if left is not None:
        ...
        if left > 0:
            arr = jnp.expand_dims(arr, tuple(range(left)))
        elif left < 0:
            arr = arr.reshape(arr.shape[-left:])
    if right is not None:
        # Add on right as required
        if right > 0:
            arr = jnp.expand_dims(arr, tuple(range(-right, 0)))
        elif right < 0:
...
def right_to(arr: array_like, ndim: int) -> jax.Array:
    """
...
def left_to(arr: array_like, ndim: int) -> jax.Array:
    """
...
def to_axis(arr: array_like, axis: int = 0, ndim: Optional[int] = None) -> jax.Array:
    """
