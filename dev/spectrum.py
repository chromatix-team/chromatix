...
class MonochromaticSpectrum(AbstractSpectrum, strict=True):
    ...
    def __init__(self, wavelength: float | Array):
        ...
        assert self.wavelength.shape == (1,), f"Monochromatic must be scalar, got {self.wavelength.shape}"
...
class PolyChromaticSpectrum(AbstractSpectrum, strict=True):
    ...
    def __init__(self, wavelength: float | Array, density: float | Array | None = None):
        ...
        self.wavelength = eqx.error_if(wavelength, wavelength.ndim != 1, "Wavelength must be 1D for polychromatic spectrum.")

        if density is None:
            self.density = jnp.ones_like(self.wavelength)
        else:
            density= jnp.asarray(density)
            self.density= eqx.error_if(density, density.shape != self.wavelength.shape, f"Density shape {density.shape} must match wavelength shape {self.wavelength.shape}.")


def Spectrum(wavelength: float | Array, density: Array | None = None) -> AbstractSpectrum:
